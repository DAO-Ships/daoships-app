...
// 2-button connect modal: Pelagus/Blip Pay (injected) + WalletConnect (QR)
    ...
    <Modal isOpen={open} onClose={() => setOpen(false)} title="Connect Wallet" size="sm">
      <div className="flex flex-col gap-3">
        <button
          ...
          disabled={busy !== null}
          className="flex items-center gap-4 p-4 rounded-lg border-2 border-dao-border hover:border-primary-500 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        >
          <div className="flex -space-x-2 flex-shrink-0">
            <img src={PELAGUS_ICON} alt="" className="w-10 h-10 rounded ring-2 ring-dao-dark-2 relative z-10" />
            <BlipPayIcon className="w-10 h-10 rounded bg-white ring-2 ring-dao-dark-2 p-1" />
          </div>
          <div className="flex-1 text-left">
            <div className="font-semibold text-dao-text">Pelagus or Blip Pay</div>
            <div className="text-xs text-dao-text-muted">
              Pelagus browser extension or Blip Pay mobile app
            </div>
          </div>
          {busy === 'injected' && (
            <span className="text-xs text-dao-text-muted">Connecting…</span>
          )}
        </button>
        ...
        {hasWalletConnect && (
          <button
            ...
            className="flex items-center gap-4 p-4 rounded-lg border-2 border-dao-border hover:border-primary-500 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <div className="w-10 h-10 rounded bg-[#3b99fc] flex items-center justify-center flex-shrink-0">
              <svg aria-hidden="true" className="w-6 h-6 text-white" viewBox="0 0 40 40" fill="currentColor">
            ...
            <div className="flex-1 text-left">
              <div className="font-semibold text-dao-text">WalletConnect</div>
              <div className="text-xs text-dao-text-muted">
        ...
        {error && (
          <div className="text-xs text-red-400 px-1 pt-2">{error}</div>
        )}
      </div>
    </Modal>
  )
